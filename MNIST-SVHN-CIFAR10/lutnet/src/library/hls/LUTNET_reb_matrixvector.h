...
ap_uint<PRECISION_REB*M> PRUNaiveREBNET(ap_uint<MACS> in[PRECISION_REB], const ap_uint<32> *weights_1, const ap_fixed<PopCountWidth, PopCountIntWidth> *thresh, const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const ap_fixed<PopCountWidth, PopCountIntWidth> *next_layer_means, const ap_uint<32> *pruning_mask, bool pt) {
      ...
      for (int ti=0; ti<K; ti++){
        for (int tj=0; tj<K; tj++){
          ...
          if (ti==K-1 && tj==K-1){
            ...
            tmp_out = PRUNaiveREBNET<PRECISION_REB, N*K*K, PopCountWidth, PopCountIntWidth, M> (tmp_in_reb, weights_1, thresh, alpha, next_layer_means, pruning_mask, pt);
