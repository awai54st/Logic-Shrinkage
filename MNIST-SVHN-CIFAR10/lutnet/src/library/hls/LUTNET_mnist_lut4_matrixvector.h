...
// Note: IN our network classifying MNIST, there exist multiple layers with equal i/o channel numbers. 
// Therefore layerids are passed in as identifiers, so taht HLS doesn't get confused.

template<unsigned int MACS, unsigned int M, unsigned int LAYERID>
void LUTARRAY_b0_MNIST(ap_uint<MACS> in, ap_uint<MACS> in_1, ap_uint<MACS> in_2, ap_uint<MACS> in_3, ap_uint<MACS> *lut_out){
#pragma HLS inline off
        for (int tm = 0; tm < M; tm++) {
        #pragma HLS UNROLL
            lut_out[tm] = in ^ in_1 ^ in_2 ^ in_3 ^ LAYERID;
        }
}

template<unsigned int MACS, unsigned int M, unsigned int LAYERID>
void LUTARRAY_b1_MNIST(ap_uint<MACS> in, ap_uint<MACS> in_1, ap_uint<MACS> in_2, ap_uint<MACS> in_3, ap_uint<MACS> *lut_out){
#pragma HLS inline off
        for (int tm = 0; tm < M; tm++) {
        #pragma HLS UNROLL
            lut_out[tm] = in ^ in_1 ^ in_2 ^ in_3 ^ LAYERID;
...
template<unsigned int PRECISION_REB, unsigned int MACS, unsigned int PopCountWidth, unsigned int PopCountIntWidth, unsigned int M, unsigned int LAYERID>
ap_uint<PRECISION_REB*M> Naive4LUT_MNIST(ap_uint<MACS> in[PRECISION_REB], ap_uint<MACS> in_1[PRECISION_REB], ap_uint<MACS> in_2[PRECISION_REB], ap_uint<MACS> in_3[PRECISION_REB], const ap_fixed<PopCountWidth, PopCountIntWidth> *thresh, const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const ap_fixed<PopCountWidth, PopCountIntWidth> *next_layer_means, bool pt) {
        #pragma HLS latency min=4 max=7
        ...
        LUTARRAY_b0_MNIST<MACS, M, LAYERID>(in[0], in_1[0], in_2[0], in_3[0], lut_out_b0);

        ap_uint<MACS> lut_out_b1[M];
        #pragma HLS ARRAY_PARTITION variable=lut_out_b1 complete dim=1
        LUTARRAY_b1_MNIST<MACS, M, LAYERID>(in[1], in_1[1], in_2[1], in_3[1], lut_out_b1);
...
//            if (tc==outCol-1) pt = 1;
//            else pt = 0;
            tmp_out = Naive4LUT_MNIST<PRECISION_REB, N*K*K, PopCountWidth, PopCountIntWidth, M, LAYERID> (tmp_in_reb, tmp_in_1_reb, tmp_in_2_reb, tmp_in_3_reb, thresh, alpha, next_layer_means, pt);
