...
void LUTARRAY_b0(ap_uint<MACS> in, ap_uint<MACS> in_1, ap_uint<MACS> *lut_out){
...
template<unsigned int MACS, unsigned int M>
void LUTARRAY_b1(ap_uint<MACS> in, ap_uint<MACS> in_1, ap_uint<MACS> *lut_out){
#pragma HLS inline off
        for (int tm = 0; tm < M; tm++) {
        #pragma HLS UNROLL
            lut_out[tm] = in ^ in_1;
...
ap_uint<PRECISION_REB*M> Naive2LUT(ap_uint<MACS> in[PRECISION_REB], ap_uint<MACS> in_1[PRECISION_REB], const ap_fixed<PopCountWidth, PopCountIntWidth> *thresh, const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const ap_fixed<PopCountWidth, PopCountIntWidth> *next_layer_means, const unsigned int *rand_map, bool pt) {
          ...
          for (int tj=0; tj<K; tj++){
            ...
            if (ti==K-1 && tj==K-1){
              for (int tb = 0; tb < PRECISION_REB; tb++){
                  ...
                  for (int t_mac = 0; t_mac < N*K*K; t_mac++){
                      #pragma HLS UNROLL
                      tmp_in_1_reb[tb].range(t_mac, t_mac) = tmp_in_reb[tb].range(rand_map[t_mac], rand_map[t_mac]);
                  }

              }
              if (tc==outCol-1) pt = 1;
              else pt = 0;
              tmp_out = Naive2LUT<PRECISION_REB, N*K*K, PopCountWidth, PopCountIntWidth, M> (tmp_in_reb, tmp_in_1_reb, thresh, alpha, next_layer_means, rand_map, pt);
