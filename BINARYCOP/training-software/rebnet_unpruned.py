...
def rebnet_unpruned(dataset, K, get_model, pruning_percentage, output_path, custom_rand_seed, training_phase):
	...
	if training_phase == 1:
		...
		Train = True; REG = True; Retrain = False; LUT = False; BINARY = True; LOGIC_SHRINKAGE = False; trainable_means = True; Evaluate = True
		acc = train_lutnet(dataset, get_model, pruning_percentage, K, Train, REG, Retrain, LUT, BINARY, LOGIC_SHRINKAGE, trainable_means, Evaluate, bnnEpochs, bnnBatchSize, output_path, custom_rand_seed)
		logger.info("Finished training bnn from scratch. ")

	elif training_phase == 0: # evaluation mode

		logger.info("Started evaluation. ")
		Train = False; REG = False; Retrain = False; LUT = False; BINARY = True; LOGIC_SHRINKAGE = False; trainable_means = False; Evaluate = True
