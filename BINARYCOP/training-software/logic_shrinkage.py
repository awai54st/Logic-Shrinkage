...
def logic_shrinkage(dataset, get_model, K, activation_pruning_percentage, output_path, custom_rand_seed):

	if dataset in ["CIFAR-10", "SVHN"]:
		ls_layers = [b'binary_conv_6']	
	elif dataset == "BinaryCoP":
		#ls_layers = [b'binary_conv_2', b'binary_conv_3', b'binary_conv_4', b'binary_conv_5', b'binary_conv_6', b'binary_dense_1', b'binary_dense_2']
		ls_layers = [b'binary_conv_6']
	elif dataset == "mu-BinaryCoP":
		#ls_layers = [b'binary_conv_2', b'binary_conv_3', b'binary_conv_4', b'binary_conv_5', b'binary_dense_1']
		ls_layers = [b'binary_conv_5']
	elif dataset == "MNIST":
	...
	for key in shrinked['model_weights'].attrs['layer_names']:
		for ls_layer in ls_layers:
			...
			if ls_layer in key:
				logger.info("ls_layer is "+str(ls_layer))
				#Load shrinkage map
				...
				#logger.info(bool_all_elems_equal)
				#logger.info(np.sum(bool_all_elems_equal))
				...
				logger.info("Layer: "+str(key))
				logger.info("Parameter count: "+str(np.sum(param_count)))
				logger.info("LUT count: "+str(np.sum(pruning_mask)))
				logger.info("Act count: "+str(np.sum(k_count)))
				
				print("Layer: "+str(key))
				print("Parameter count: "+str(np.sum(param_count)))
				print("LUT count: "+str(np.sum(pruning_mask)))
				print("Act count: "+str(np.sum(k_count)))
