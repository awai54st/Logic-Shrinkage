...
def lutnet_init(dataset, K, get_model, output_path, custom_rand_seed):

	if dataset in ["CIFAR-10", "SVHN"]:
		target_layers = [b'binary_conv_6']
	elif dataset == "BinaryCoP":
		#target_layers = [b'binary_conv_2', b'binary_conv_3', b'binary_conv_4', b'binary_conv_5', b'binary_conv_6', b'binary_dense_1', b'binary_dense_2']
		target_layers = [b'binary_conv_6']
	elif dataset == "mu-BinaryCoP":
		#target_layers = [b'binary_conv_2', b'binary_conv_3', b'binary_conv_4', b'binary_conv_5', b'binary_dense_1']
		target_layers = [b'binary_conv_5']
	elif dataset == "MNIST":
	...
	for key in bl['model_weights'].attrs['layer_names']:
		...
		if any(target_layer in key for target_layer in target_layers):
			...
			logger.debug("pret_c_param shape is "+str(np.shape(pret_c_param)))

			weight_shape = np.shape(bl_w1)
			
			if b'binary_conv' in key:
			...
			if K > 3:
				...
				pruning_mask = np.logical_or(pruning_mask, pruning_mask_recover)
				init_mask = np.reshape(init_mask, weight_shape)
				logger.debug("init_mask shape is "+str(np.shape(init_mask)))
				...
				logger.debug("c_param shape is "+str(np.shape(c_param)))
			...
			logger.debug("RHS shape is "+str(np.shape(np.array(np.stack(c_param, axis=0), dtype=float))))
