...
void LUTARRAY_b0(ap_uint<MACS> in, ap_uint<MACS> in_1, ap_uint<MACS> in_2, ap_uint<MACS> in_3, ap_uint<MACS> in_4, ap_uint<MACS> *lut_out){
#pragma HLS inline off
        for (int tm = 0; tm < M; tm++) {
        #pragma HLS UNROLL
            lut_out[tm] = in ^ in_1 ^ in_2 ^ in_3 ^ in_4;
        }
}

template<unsigned int MACS, unsigned int M>
void LUTARRAY_b1(ap_uint<MACS> in, ap_uint<MACS> in_1, ap_uint<MACS> in_2, ap_uint<MACS> in_3, ap_uint<MACS> in_4, ap_uint<MACS> *lut_out){
#pragma HLS inline off
        for (int tm = 0; tm < M; tm++) {
        #pragma HLS UNROLL
            lut_out[tm] = in ^ in_1 ^ in_2 ^ in_3 ^ in_4;
...
ap_uint<PRECISION_REB*M> Naive5LUT(ap_uint<MACS> in[PRECISION_REB], ap_uint<MACS> in_1[PRECISION_REB], ap_uint<MACS> in_2[PRECISION_REB], ap_uint<MACS> in_3[PRECISION_REB], ap_uint<MACS> in_4[PRECISION_REB], const ap_fixed<PopCountWidth, PopCountIntWidth> *thresh, const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const ap_fixed<PopCountWidth, PopCountIntWidth> *next_layer_means, bool pt) {
        ...
        LUTARRAY_b0<MACS, M>(in[0], in_1[0], in_2[0], in_3[0], in_4[0], lut_out_b0);

        ap_uint<MACS> lut_out_b1[M];
        #pragma HLS ARRAY_PARTITION variable=lut_out_b1 complete dim=1
        LUTARRAY_b1<MACS, M>(in[1], in_1[1], in_2[1], in_3[1], in_4[0], lut_out_b1);
...
//#pragma HLS DATAFLOW
  ...
  ap_uint<N*K*K> tmp_in_4_reb [PRECISION_REB];
  #pragma HLS ARRAY_PARTITION variable=tmp_in_4_reb complete dim=1
          ...
          if (ti==K-1 && tj==K-1){
            for (int tb = 0; tb < PRECISION_REB; tb++){
                ...
                for (int t_mac = 0; t_mac < N*K*K; t_mac++){
                    ...
                    tmp_in_4_reb[tb].range(t_mac, t_mac) = tmp_in_reb[tb].range(rand_map_3[t_mac], rand_map_3[t_mac]);
                }
            }
            if (tc==outCol-1) pt = 1;
            else pt = 0;
            tmp_out = Naive5LUT<PRECISION_REB, N*K*K, PopCountWidth, PopCountIntWidth, M> (tmp_in_reb, tmp_in_1_reb, tmp_in_2_reb, tmp_in_3_reb, tmp_in_4_reb, thresh, alpha, next_layer_means, pt);
